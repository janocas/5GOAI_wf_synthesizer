...
// - - - - - - - - - - - - - - - - - - - - - - - - - - 
// ER: Not adding OAI headers to samples
#if 0
...
//          fullwrite(b->conn_sock, samplesVoid[0], sampleToByte(nsamps, nbAnt), t);
            fullwrite(1, samplesVoid[0], sampleToByte(nsamps, nbAnt), t);
        }
      } else {
...
//          fullwrite(b->conn_sock, (void *)tmpSamples, sampleToByte(nsamps, nbAnt), t);
        fullwrite(1, (void *)tmpSamples, sampleToByte(nsamps, nbAnt), t);
//        }
      } // else
//  } // if (nbAnt == 1) 
//} // for (int i = 0; i < MAX_FD_RFSIMU; i++)
...
static bool flushInput(rfsimulator_state_t *t, int timeout, int nsamps_for_initial) {
  ...
  // - - - - - - - - - - - - - - - - - -
  // ER: Do not need to wait for socket
  // when genrating a file
int nfds = 0;
