...
// - - - - - - - - - - - - - - - - - - - - - - - - 
// ER: Added to control Waveform generation based
// on number of frames. after print IQ, we signal the atached thread.
void threadCreate_noDetached(pthread_t* t, void * (*func)(void*), void * param, char* name, int affinity, int priority){
  pthread_attr_t attr;
  int ret;
