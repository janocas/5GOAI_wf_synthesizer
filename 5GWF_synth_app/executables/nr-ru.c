...
void ru_tx_func(void *param) {
  ...
  if(!emulate_rf && (oai_exit == 0)) {
    // do outgoing fronthaul (south) if needed
    if ((ru->fh_north_asynch_in == NULL) && (ru->fh_south_out)) ru->fh_south_out(ru,frame_tx,slot_tx,info->timestamp_tx);
    //- - - - - - - - - - - - - - - - - - - - - - 
    //ER: Tx start at frame 0 slot 6 we finish Tx 
    //    at frame 0 slot 11. = 1024 frames
    //- - - - - - - - - - - - - - - - - - - - - -
    if (RC.nframes != -1) {
      LOG_I(PHY,"Number of frames: %d and slots %d\n", frame_tx, slot_tx);
      if ((frame_tx == RC.nframes) && slot_tx == 11) {
        LOG_W(PHY,"Last symbol written into IQ File... RU entering iddle state...\n");
        oai_exit = 1;
      }
    }
